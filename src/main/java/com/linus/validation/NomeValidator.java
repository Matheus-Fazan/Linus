...
public class NomeValidator implements ParamValidator {

    @Override
    public void validate(String value) throws ParamException {
        if (isEmptyString(value)) throw new EmptyParamException("nome");
        if (isOutOfBoundString(value)) throw new OutOfBoundsParamException("nome");
    }
}
